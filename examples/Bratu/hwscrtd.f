...
c
c  This is the fast Poisson solver used with the nitsol test codes. 
c
c  This code was originally single precision. Quick and dirty conversion 
c  to double precision done on 1/30/90.                            --HFW
c  Further mods to change all declarations of arrays of length 1 to 
c  assumed-size arrays.  3/96                                      --MAP
...
C     I/O            NONE
C
C     PRECISION      SINGLE --- Converted to double ... HFW, 1/30/90.
C
...
C
  138 A1 = 1.d0
      A2 = 1.d0
      IF (NBDCND .EQ. 3) A2 = 2.d0
      IF (MBDCND .EQ. 3) A1 = 2.d0
      S1 = 0.d0
      MSP1 = MSTART+1
      MSTM1 = MSTOP-1
      NSP1 = NSTART+1
  ...
  141 CONTINUE
     ...
     1     F(MSTOP,NSTOP)
      S = (2.d0+dfloat(NUNK-2)*A2)*(2.d0+dfloat(MUNK-2)*A1)
      PERTRB = S1/S
